...
DECLARE
  r record;
  _branch_name text;
  _target text := COALESCE(NEW.target_type, 'all');
...
BEGIN
  ...
  LOOP
    -- In-app notification (SECURITY DEFINER bypasses RLS so parent rows get inserted
    -- even when the announcer is not in the parent's branch_memberships).
