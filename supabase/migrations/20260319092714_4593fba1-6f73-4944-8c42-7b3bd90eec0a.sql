...
-- Attach the validation trigger (BEFORE INSERT OR UPDATE on payments)
...
-- Attach the sync trigger (AFTER INSERT, UPDATE, DELETE on payments)
...
WHERE inv.id = sub.invoice_id
  AND (inv.amount_paid != COALESCE(sub.total_paid, 0)
...
-- Also fix invoices with no payments but wrong status (e.g. stuck as partial/paid)
UPDATE public.invoices inv
SET amount_paid = 0,
